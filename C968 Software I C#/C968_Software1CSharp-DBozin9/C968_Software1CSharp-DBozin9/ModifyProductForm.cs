...
namespace C968_Software1CSharp_DBozin9
{
    ...
    {
        public ModifyProductForm(int i)
        {
            InitializeComponent();
            ...
            //Apply selected product's variables to form text boxes
            IDTextBox.Text = Convert.ToString(Program.inventory.Products[i].ProductID);
            NameTextBox.Text = Program.inventory.Products[i].Name;
            InvTextBox.Text = Convert.ToString(Program.inventory.Products[i].InStock);
            PriceTextBox.Text = Convert.ToString(Program.inventory.Products[i].Price);
            MinTextBox.Text = Convert.ToString(Program.inventory.Products[i].Min);
            MaxTextBox.Text = Convert.ToString(Program.inventory.Products[i].Max);
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var product = new Product(Convert.ToInt32(IDTextBox.Text), NameTextBox.Text, Convert.ToDouble(PriceTextBox.Text), Convert.ToInt32(InvTextBox.Text), Convert.ToInt32(MinTextBox.Text), Convert.ToInt32(MaxTextBox.Text));
                Program.inventory.Products[Convert.ToInt32(IDTextBox.Text)] = product;
        ...
        private void UpdateLists()
        {
            ...
            //Loop through all associated parts and add to list view
            foreach (Part aPart in Program.inventory.Products[Convert.ToInt32(IDTextBox.Text)].associatedParts)
            {
                ProductPartsList.Items.Add((ListViewItem)SearchedPartsList.Items[aPart.PartID].Clone());
            }
        }
    }
}