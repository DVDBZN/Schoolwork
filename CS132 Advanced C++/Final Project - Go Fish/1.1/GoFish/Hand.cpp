...
void Hand::findFour()
{
	...
	if (cardsInHand.size() >= 4)
	{
		for (int i = 0; i < cardsInHand.size() - 3; i++)
		{
			...
			{
				cout << "A set of " << cardsInHand[i].getRank() << "s has been placed on the table.\n\n";
...
string Hand::chooseCard()
{
	...
	if (cardsInHand.size() >= 3)
	{
		...
		//If both previous choices are equal and the next one will be too, then pick other card
		if (previousChoice == choice && match.size() == 1)
		{
			//Do nothing
		}
