...
#This function is the only new addition to the code
#However some other code has been updated and improved
def search():
    search = raw_input("\tEnter your search: ").lower()

    #Find mathing results
    select = ("SELECT * FROM Menu WHERE Name LIKE '%" + search + "%'")
    cur.execute(select)
...
def display(settings):
    ...
    select = ("SELECT * FROM Menu %s ORDER BY %s %s %s" %
              (where, settings[0], settings[1], limit))
