...
def display():
    ...
    try:
        choice = int(raw_input("\t1...Customers\n\t2...Orders\n" +
                               "\t3...Orders by Customer First Name\n" +
                               "\t4...Orders by Customer Last Name\n"))
    ...
    if choice == 1:
        ...
        #Table header                           Print hyphen 79 times
    ...
    #Orders table
        ...
        for row in cur.fetchall():
            print "{0:^9}|{1:^12}|{2:^32}|{3:^7}".format(str(row[0]),
                                                        ...
                                                        row[2],
                                                        str(" $%.2f" % row[3]))
    #Orders by Customer's first name
    elif choice == 3:
        firstName = raw_input("Enter customer first name: ")

        #SELECT all columns, JOIN both tables, WHERE first name is LIKE input
        select = ("SELECT Customer.CustID, Customer.FirstName, Customer.LastName, " +
                  "Orders.OrderID, Orders.ItemDescript, Orders.Price " +
                  "FROM Customer " +
                  "INNER JOIN Orders ON Customer.CustID=Orders.CustomerID " +
        ...
        #If selected data is empty, notify user 
        if cur.fetchone() is None:
            print "\nNo purchases made by customer\n"
            return

        cur.execute(select)

        #Header for table
        print "\n{0:^10}|{1:^10}|{2:^9}|{3:^7}|{4:^28}|{5:^7}".format("CustomerID",
                                                                    "First Name",
                                                                    "Last Name",
                                                                    "OrderID",
                                                                    "Item Description",
        ...
        #Table
        for row in cur.fetchall():
            print "{0:^10}|{1:^10}|{2:^9}|{3:^7}|{4:^28}|{5:^7}".format(row[0],
                                                                        row[1],
                                                                        row[2],
                                                                        row[3],
                                                                        row[4],
        ...
        lastName = raw_input("Enter customer last name: ")

        #SELECT all columns, JOIN both tables, WHERE last name is LIKE input
        select = ("SELECT Customer.CustID, Customer.FirstName, Customer.LastName, " +
                  "Orders.OrderID, Orders.ItemDescript, Orders.Price " +
                  "FROM Customer " +
                  "INNER JOIN Orders ON Customer.CustID=Orders.CustomerID " +
